...
const IpTracker = () => {
    ...
    const getFormattedRawText = () => {
        ...
        // 해외 IP인 경우 백엔드가 생성해준 rawText를 우선 반환
            ...
            <div className="flex flex-col gap-6 fade-in-enter">
                ...
                <div className="flex justify-between items-center" style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                    <div className="flex gap-2">
                        ...
                        <span className="whois-badge badge-kr">국가: {whois.countryCode}</span>
                    </div>
                ...
                <div className="whois-card-grid">
                    {/* PI/ISP 할당 정보 */}
                    {activeBlock.PI && renderIpBlockDetails(activeBlock.PI, `${activeLang === 'ko' ? 'ISP / 할당 대행 기관 정보' : 'ISP Allocation Provider Info'}`)}
    ...
    const renderResultSection = () => {
        ...
        // 해외 IP 판정 (백엔드에 rdap 블록이 들어있거나, registry가 KRNIC이 아니면서 korean/english가 없는 경우)
        const isForeignIp = whois.queryType && whois.queryType.includes('IP') && (whois.rdap || (whois.registry !== 'KRNIC' && !whois.korean && !whois.english));
